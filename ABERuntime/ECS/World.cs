namespace ABEngine.ABERuntime.ECS
{
    ...
    public partial struct World : IEquatable<World>, IEnumerable<Archetype>
    {
        ...
        /// <summary>
        /// Adds a callback to be invoked whenever an entity is disabled
        /// </summary>
        /// <param name="callback">callback to invoke</param>
        /// <param name="register">set true to add callback, false to remove the callback</param>
        public World OnDisable<Component>(DisableEntityEvent<Component> callback, bool register = true)
        {
            if (this.TryGetWorldInfo(out var world_info))
            {
                var data = world_info.GetData<Component>();
                if (register)
                    data.disable_callback += callback;
                else data.disable_callback -= callback;
                data.has_disable_callback = data.disable_callback != null;
            }
            return this;
        }
        ...
        /// <param name="callback">callback to invoke</param>
        /// <param name="register">set true to add callback, false to remove the callback</param>
        public World OnEnable<Component>(EnableEntityEvent<Component> callback, bool register = true)
        {
            if (this.TryGetWorldInfo(out var world_info))
            {
                var data = world_info.GetData<Component>();
                if (register)
                    data.enable_callback += callback;
                else data.enable_callback -= callback;
                data.has_enable_callback = data.enable_callback != null;
            }
            return this;
        }
...
namespace ABEngine.ABERuntime.ECS.Internal
{
    using System.Collections.Generic;
    ...
    public delegate void DisableEntityEvent<T>(Entity entity, T component);
    public delegate void EnableEntityEvent<T>(Entity entity, T component);
    ...
    {
        public bool has_remove_callback, has_set_callback, has_disable_callback, has_enable_callback, assigned_data;
        ...
        public abstract void InvokeDisableCallback(in Entity entity, in object component);
        public abstract void InvokeEnableCallback(in Entity entity, in object component);
    ...
    public sealed class WorldData<T> : WorldData
    {
        ...
        public DisableEntityEvent<T> disable_callback;
        public EnableEntityEvent<T> enable_callback;
        ...
        public override void InvokeDisableCallback(in Entity entity, in object comp)
           => disable_callback?.Invoke(entity, (T)comp);

        public override void InvokeEnableCallback(in Entity entity, in object comp)
          => enable_callback?.Invoke(entity, (T)comp);
    ...
    public struct StructureEventHandler
    {
        ...
        public void Disable(in Entity entity)
        {
            ...
            {
                var arch = entity_info.arch_info;

                (WorldData callback, object value)[] disabled =          // this causes allocations
                    new (WorldData, object)[arch.component_count];  // but other means are quite convuluted
                int length = 0;

                for (int i = 0; i < arch.component_count; ++i)
                {
                    var pool = arch.component_buffers[i];
                    var callback = world.GetData(pool.type_id);
                    if (callback.has_disable_callback)
                    {
                        disabled[length] = (callback, pool.buffer.array[entity_info.arch_index]); // this causes boxing :(
                        length++;
                    }
                }

                for (int i = 0; i < length; ++i)
                    disabled[i].callback.InvokeDisableCallback(entity, disabled[i].value);
            }
        }

        public void Enable(in Entity entity)
        {
            ref var entity_info = ref Entities.All[entity.index];
            if (entity_info.version == entity.version)
            {
                ...
                (WorldData callback, object value)[] enabled =          // this causes allocations
                    new (WorldData, object)[arch.component_count];  // but other means are quite convuluted
                int length = 0;
                ...
                for (int i = 0; i < arch.component_count; ++i)
                {
                    var pool = arch.component_buffers[i];
                    var callback = world.GetData(pool.type_id);
                    if (callback.has_enable_callback)
                    {
                        enabled[length] = (callback, pool.buffer.array[entity_info.arch_index]); // this causes boxing :(
                        length++;
                    }
                }

                for (int i = 0; i < length; ++i)
                    enabled[i].callback.InvokeEnableCallback(entity, enabled[i].value);
            }
        }
