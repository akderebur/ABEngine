...
namespace ABEngine.ABERuntime.ECS
{
    ...
    public partial class Query : IEnumerable<Archetype>
    {
        ...
        /// <summary>
        /// Can have any of this components
        /// </summary>
        public Query HasAny<T>()
        {
            archetype_count = 0;
            structure_update = -1;
            include.Add<T>();
        ...
        /// <summary>
        /// filters entities to those that have any of these components
        /// </summary>
        public Query HasAny(params Type[] types)
        {
            archetype_count = 0;
            structure_update = -1;
            include.Add(types);
        ...
        /// filters entities to those that have any of these components
        /// </summary>
        public Query HasAny(IEnumerable<Type> types)
        {
            if (types != null)
                foreach (var type in types)
                    HasAny(type);
        ...
        bool Update(out World_Info world_info)
        {
            if (world.TryGetWorldInfo(out world_info))
            {
                ...
                for (; last_lookup < world_info.archetype_terminating_index; ++last_lookup)
                {
                    ...
                    bool includeCond = false;
                    if (isAnyQuery)
                        includeCond = arch.signature.HasAny(include);
                    else
                        includeCond = arch.signature.HasAll(include);

                    if (includeCond && !arch.signature.HasAny(exclude))
                    {
